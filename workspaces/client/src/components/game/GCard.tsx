...
export default function GCard({payload}: Props) {
  ...
  const [{isDragging, canDrag}, dragRef, dragPreview] = useDrag(() => ({
    type: 'card',
    item: payload,
    canDrag: (monitor) => {
      if(payload.cost<0)return payload.exhausted==false && payload.attack>0 && payload.owner==currentLobbyState.turn;
      return currentLobbyState.hero[payload.owner].mana >= payload.cost;
    },
    collect: (monitor) => ({
  ...
  return isDragging ? (<></>) : (
    <div className='relative group flex flex-col overflow-visible' style={{ height: 80, width:60, }}>
      ...
      <div
        className={`transition relative py-3 flex justify-center`} ref={dragRef} style={{ height: 80, width:60, }}
      >
        ...
        {currentLobbyState.hero[payload.owner].mana < payload.cost && <div className='absolute right-0 top-0 bottom-0 left-0 z-30 bg-yellow-600 opacity-50'/>}
      ...
      <div className="group-hover:opacity-100 transition-opacity bg-gray-800 px-1 text-sm text-gray-100 rounded-md absolute 
      opacity-0 m-4 h-10 mx-auto w-40 top-100 -translate-y-12 z-50">
        <span className='line-clamp-2'>
          {payload.type} {payload.type=='SPELL'? payload.text:''}
        </span>
      </div>
    </div>
  );
